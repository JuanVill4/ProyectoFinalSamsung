...
def mem_usage():
    ...
    return "Memoria: {:.0f} MB {:.0f}%".format(usage.used / (1024 * 1024), usage.percent)

# Función para obtener el uso del disco
def disk_usage(dir):
    usage = psutil.disk_usage(dir)
    return "SD: {:.2f} GB {:.0f}%".format(usage.used / (1024 * 1024 * 1024), usage.percent)
...
while True:
    ...
    with canvas(device) as draw:
        ...
        draw.text((0, 16), mem_usage(), fill="white")
        draw.text((0, 30), disk_usage('/'), fill="white")
        draw.text((0, 50), str(psutil.virtual_memory()), fill="white")
